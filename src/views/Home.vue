...
import LeftNav from '@/components/homeComponents/LeftNav.vue';
import RightNav from '@/components/homeComponents/RightNav.vue';
...
const toggleMessageEvent = (messageId: string) => {
    isMessageEventMap[messageId] = !isMessageEventMap[messageId];
};
...
async function sendMessage() {
    ...
    const tempMsg: TestMessage = {
        ...
        receiverID: currentReceiverID.value as string, // set your receiver dynamically
...
<template>
    ...
    <section :class="[
        ...
        <!-- Left Nav -->
        <LeftNav />
        <div class="flex flex-row h-screen w-full ml-1" :class="isDark ? 'bg-black' : 'bg-white'">
            <div :class="isDark ? 'bg-blue-300/30' : 'bg-blue-400'" class="w-full h-full rounded-lg mr-2 flex flex-col">
                <Streaming />
                <template ref="messageContainer" @scroll="handleScroll" :class="!endCall ? 'hidden' : 'flex'"
                    class="flex-col rounded-lg h-screen overflow-y-auto">
                    <div v-for="msg in messages" :key="msg._id" :id="msg._id" class="flex flex-col space-y-2"
                        ...
                        <div class="relative flex flex-row my-0">
                            <template v-if="msg.senderID === currentUser">
                                <MessageEvent v-if="isMessageEventMap[msg._id as any]"
                                    class="right-full -translate-y-35" />
                                <EllipsisVertical :size="25" @click="toggleMessageEvent(msg._id)" color="black"
                            ...
                            <template v-else>
                                ...
                                <EllipsisVertical :size="25" @click="toggleMessageEvent(msg._id)" color="black"
                                    class="flex self-center cursor-pointer" />
                                <MessageEvent v-if="isMessageEventMap[msg._id as any]"
                                    class="left-full -translate-y-35" />
                            </template>
                        </div>
                ...
                <div class="p-4 cursor-pointe bottom-0 items-center space-x-3" :class="[isDark ? 'text-gray-200 bg-gray-800' : 'text-black bg-blue-400',
                !endCall ? 'hidden' : 'flex']">
